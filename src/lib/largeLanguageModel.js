...
const MCP_SERVER_URL =
  "https://server-service-912224113379.us-central1.run.app/api/mongodb";
const GOOGLE_SEARCH_MCP_SERVER =
  "https://server-service-912224113379.us-central1.run.app/api/google";
...
async function sendWithRetry(chat, userMessageContent, {
  retries = 3,
  initialDelayMs = 500,
  backoffFactor = 2,
} = {}) {
...
async function handleUserQuery(chat, userMessageContent) {
  ...
  if (!finalAnswer && safetyCounter > 0) {
    try {
      ...
      if (lastFunctionResult) {
        return formatFunctionResultsDirectly(lastFunctionName, lastFunctionResult, userMessageContent);
      }
  ...
  return finalAnswer || "I apologize, but I couldn't process your request. Please try again.";
}
    ...
    if (functionName === "googleSearch") {
      ...
      if (result.contents && Array.isArray(result.contents)) {
        ...
        result.contents.forEach((content, index) => {
          response += `Source ${index + 1}: ${content.url || 'Unknown source'}\n`;
          response += `${content.content.substring(0, 300)}${content.content.length > 300 ? '...' : ''}\n\n`;
        });
        return response;
      }
      ...
      if (Array.isArray(result) && result.length > 0) {
        return `I found ${result.length} results in the database. Here's a summary:\n\n${JSON.stringify(result.slice(0, 3), null, 2)}${result.length > 3 ? '\n\n...and more results.' : ''}`;
      }
    } else if (functionName === "listModels") {
      // Format models list
      if (Array.isArray(result) || (result.models && Array.isArray(result.models))) {
...
Preferred Currency: ${session.user.currency}
`;
  }
