      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-[80%] max-w-md bg-white shadow-xl z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-slate-50 to-white border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">{sources.length} references</p>
                </div>
              </div>
              <motion.button
                onClick={onClose}
            ...
            {/* Sources list */}
            <motion.div 
              className="flex flex-col overflow-y-auto overflow-x-hidden bg-white"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <div className="w-full">
                {sources.map((source, index) => (
                  ...
                  >
                    <div className="group/card cursor-pointer relative bg-white p-6 transition-all duration-200 
                      hover:border-blue-200 hover:bg-gray-200 border-b border-b-gray-200"
                    >
                      {/* Title */}
                      {source.title && (
                        <h3 className="font-regular text-[14px] text-slate-900 line-clamp-3 break-words">
                          {source.title}
                        </h3>
                      )}
                      ...
                      {/* Link preview */}
                      {source.url && (
                        <div className="mt-2 inline-flex items-center gap-1.5 text-sm text-blue-600 group-hover/card:text-blue-700">
